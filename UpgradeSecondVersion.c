...
// Function to display all books with numbering
...
int main() {
    ...
    if (age >= 18) {
        ...
        printf(" You are not eligible for our services.\n");
        return 0; // exit early if underage
    }
    ...
    int genreValid = 0;
    int salary = 70000;
    const char *acceptedGenres[] = {"Self-Help", "Sci-fi", "Research"};
    ...
    printf("Choose your book genre (Options: Self-Help, Sci-fi, Research): ");
    fgets(bookGenre, sizeof(bookGenre), stdin);
    removeNewline(bookGenre);

    // Validate genre
    for (int i = 0; i < 3; i++) {
        if (strcmp(bookGenre, acceptedGenres[i]) == 0) {
            genreValid = 1;
            break;
        }
    }
    ...
    if (!genreValid) {
        printf("\n❌ Invalid genre. Please submit only Self-Help, Sci-fi, or Research.\n");
        return 0; // Exit if invalid genre
    }
    ...
    if (strcmp(ruleStatus, "Pass") == 0) {
        ...
        printf(" You failed the rule check. View some successful authors:\n");
        printf("- \"%s\" by %s\n", books[0].title, books[0].author);
        printf("- \"%s\" by %s\n", books[1].title, books[1].author);
        printf("- \"%s\" by %s\n", books[6].title, books[6].author);
        printf(" Note: You will need to pay a review fee.\n");
    }

    printf("\n Additional Notes:\n");
    printf(" Your title must be unique.\n");
    printf(" Your book must fit in one of these genres: ");
    for (int i = 0; i < 3; i++) {
        printf("%s", acceptedGenres[i]);
        if (i < 2) printf(", ");
    }
    printf("\n💼 Starting salary: $%d\n", salary);
